  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-border-strong bg-[#0b1013] shadow-[0_40px_120px_rgba(0,0,0,.42)]">
      <div className="flex h-12 items-center border-b border-border px-4 sm:px-5">
        ...
        <div className="ms-auto flex items-center gap-2 font-mono text-[9px] text-success"><span className="live-dot size-1.5 rounded-full bg-success text-success" />LIVE</div>
      </div>
      <div className="grid min-h-[480px] lg:grid-cols-[1fr_300px]">
        <div className="relative overflow-hidden border-b border-border p-5 sm:p-8 lg:border-b-0 lg:border-e">
          ...
          <div className="relative flex items-start justify-between gap-6">
            <div>
              <p className="eyebrow">Leading explanation</p>
              <h2 className="mt-2 max-w-xl text-balance text-xl font-medium tracking-[-0.025em] sm:text-2xl">The flag rollout is the strongest shared change.</h2>
              <p className="mt-2 text-xs leading-5 text-muted">Code, deployment, trace, and business signals agree. Third-party latency remains plausible.</p>
            </div>
            <span className="hidden rounded-full border border-success/20 bg-success/[0.07] px-2.5 py-1 font-mono text-[9px] text-success sm:block">HIGH CONFIDENCE</span>
          </div>
            ...
            {nodes.map((node, index) => {
              ...
              return (
                <div key={node.label} className="relative z-10 flex w-[118px] flex-col items-center text-center">
                  ...
                  <p className="mt-3 font-mono text-[10px] font-medium text-foreground">{node.label}</p>
                  <p className="mt-1 font-mono text-[8px] text-muted">{node.sub}</p>
            ...
            {nodes.map((node, index) => {
              ...
              return (
                <li key={node.label} className="flex items-center gap-3 rounded-lg border border-border bg-panel-soft p-3">
                  ...
                  <span><span className="block font-mono text-[10px]">{node.label}</span><span className="block text-[9px] text-muted">{node.sub}</span></span>
          ...
          <div className="relative mt-8 flex flex-wrap items-center gap-4 border-t border-border pt-4 font-mono text-[8px] text-muted">
            ...
            <span className="ms-auto">7 evidence items · updated 18s ago</span>
          </div>
        </div>
        <aside className="p-5 sm:p-6">
          <div className="flex items-center justify-between"><p className="eyebrow">Customer impact</p><span className="font-mono text-[9px] text-danger">SEV-2</span></div>
          <p className="mt-5 font-mono text-4xl font-medium tracking-[-0.06em] text-foreground">−7.3%</p>
          <p className="mt-1 text-xs text-muted">checkout conversion</p>
          ...
          <div className="mt-5 space-y-3 border-t border-border pt-5">
            {[['p95 latency','1.84 s','+171%'],['Error rate','4.9%','+4.2 pp'],['Revenue impact','−$12.4k','per hour']].map(([label,value,change]) => (
              <div key={label} className="flex items-center justify-between gap-3"><span className="text-[10px] text-muted">{label}</span><span className="ms-auto font-mono text-[10px] text-foreground">{value}</span><span className="w-12 text-end font-mono text-[8px] text-danger">{change}</span></div>
            ))}
          </div>
          <div className="mt-6 rounded-lg border border-inference/15 bg-inference/[0.045] p-3">
            <p className="flex items-center gap-2 text-[10px] font-medium"><Bot aria-hidden="true" className="size-3.5 text-inference" />Proposed action</p>
            <p className="mt-2 text-[10px] leading-5 text-muted">Disable instant-tax-v2, then watch p95 for 5 minutes.</p>
          </div>
        </aside>
      </div>
    </div>
