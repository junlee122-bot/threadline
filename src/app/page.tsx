...
import { koText } from "@/lib/locales/ko";
...
export default async function Home() {
  ...
  const t = (english: string) => locale === "ko" ? koText(english) : english;
  return (
    <div className="relative min-h-screen overflow-hidden bg-background">
      ...
      <header className="relative z-20 mx-auto flex h-20 max-w-[1240px] items-center px-5 sm:px-8">
        <Wordmark />
        <nav aria-label={t("Marketing navigation")} className="ms-auto hidden items-center gap-7 text-xs text-muted md:flex">
          <a href="#product" className="transition-colors hover:text-foreground">{t("Product")}</a>
          <a href="#principles" className="transition-colors hover:text-foreground">{t("Principles")}</a>
          <a href="#story" className="transition-colors hover:text-foreground">{t("Live story")}</a>
          <Link href="/lab" className="transition-colors hover:text-foreground">{t("Crisis Lab")}</Link>
          <a href="https://github.com/junlee122-bot/something" className="transition-colors hover:text-foreground">GitHub</a>
        </nav>
        <LanguageToggle className="ms-auto md:ms-8" />
        <Link
          href="/command"
          className="ms-2 inline-flex h-10 items-center gap-2 rounded-full bg-primary px-4 text-xs font-semibold text-primary-foreground transition-transform hover:-translate-y-0.5"
        >
          {t("Enter live demo")}
        ...
        <section className="mx-auto max-w-[1240px] px-5 pb-20 pt-20 sm:px-8 sm:pt-28 lg:pb-28 lg:pt-32">
          <div className="mx-auto max-w-4xl text-center">
            <div className="fade-up mx-auto mb-7 inline-flex items-center gap-2 rounded-full border border-border bg-panel/70 px-3 py-1.5 font-mono text-[10px] text-muted backdrop-blur">
              ...
              {t("Evidence-native software intelligence")}
              <span className="text-border-strong">/</span>
              {t("Interactive concept")}
            </div>
            <h1 className="fade-up fade-up-delay-1 text-balance text-[clamp(3.25rem,8vw,7.2rem)] font-medium leading-[0.92] tracking-[-0.065em]">
              {t("Every signal,")}
              <br />
              <span className="text-primary">{t("traced to source.")}</span>
            </h1>
            <p className="fade-up fade-up-delay-2 mx-auto mt-8 max-w-2xl text-balance text-base leading-7 text-muted sm:text-lg sm:leading-8">
              {t("Code, deployments, telemetry, and customer impact—connected into one calm, evidence-backed command center.")}
            </p>
          ...
          <div className="mx-auto mt-8 grid max-w-[1120px] grid-cols-2 border-y border-border py-5 sm:grid-cols-4">
            {[
              ...
              <div key={label} className={`px-4 py-3 text-center ${index % 2 ? "border-s border-border" : ""} sm:border-s sm:first:border-s-0`}>
                ...
                <p className="mt-1 text-[10px] text-muted">{t(label)}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="product" className="border-t border-border bg-[#0a0e10] px-5 py-24 sm:px-8 sm:py-32">
          <div className="mx-auto max-w-[1120px]">
            <div className="max-w-2xl">
              <p className="eyebrow text-primary">{t("One operating picture")}</p>
              <h2 className="mt-4 text-balance text-3xl font-medium tracking-[-0.035em] sm:text-5xl">
                {t("Stop reconstructing incidents from browser tabs.")}
              </h2>
              <p className="mt-5 max-w-xl text-sm leading-7 text-muted sm:text-base">
                {t("Threadline keeps source evidence and AI interpretation separate, then places both on the same time-aware graph.")}
              </p>
            </div>
              ...
              {features.map((feature) => {
                ...
                return (
                  <article key={feature.title} className="min-h-[280px] bg-panel p-7 sm:p-8">
                    ...
                    <p className="eyebrow mt-10">{t(feature.label)}</p>
                    <h3 className="mt-3 text-xl font-medium tracking-[-0.025em]">{t(feature.title)}</h3>
                    <p className="mt-3 text-sm leading-6 text-muted">{t(feature.copy)}</p>
                  </article>
                );
              })}
            </div>
        ...
        <section id="story" className="px-5 py-24 sm:px-8 sm:py-32">
          <div className="mx-auto grid max-w-[1120px] gap-14 lg:grid-cols-[0.82fr_1.18fr] lg:gap-24">
            <div className="lg:sticky lg:top-28 lg:self-start">
              <p className="eyebrow text-primary">{t("A complete incident story")}</p>
              <h2 className="mt-4 text-balance text-3xl font-medium tracking-[-0.035em] sm:text-5xl">
                {t("From a single commit to verified recovery.")}
              </h2>
              <p className="mt-5 text-sm leading-7 text-muted sm:text-base">
                {t("The demo replays a realistic checkout regression across source, deployment, tracing, SLO, and business data—with no API keys required.")}
              </p>
                ...
                return (
                  <li key={step.time} className="relative pb-12 last:pb-0">
                    ...
                    <div className="panel p-5 sm:p-6">
                      ...
                      <h3 className="mt-3 text-lg font-medium">{t(step.title)}</h3>
                      <p className="mt-1 text-sm text-muted">{t(step.detail)}</p>
                    </div>
                  </li>
                );
              })}
        ...
        <section id="principles" className="border-y border-border bg-panel px-5 py-24 sm:px-8">
          <div className="mx-auto max-w-[1120px]">
            <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
              <div>
                <p className="eyebrow text-primary">{t("Designed for trust")}</p>
                <h2 className="mt-4 text-3xl font-medium tracking-[-0.035em] sm:text-4xl">{t("AI that never hides the evidence.")}</h2>
              </div>
              <div className="grid gap-6 sm:grid-cols-2">
                {[
                  ...
                  <div key={title} className="border-t border-border pt-5">
                    <p className="flex items-center gap-2 text-sm font-medium"><Check aria-hidden="true" className="size-4 text-primary" />{t(title)}</p>
                    <p className="mt-2 text-xs leading-6 text-muted">{t(copy)}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="px-5 py-24 sm:px-8 sm:py-32">
          <div className="relative mx-auto max-w-[1120px] overflow-hidden rounded-2xl border border-primary/15 bg-panel p-8 sm:p-14 lg:p-20">
            ...
            <div className="relative max-w-2xl">
              <p className="eyebrow text-primary">{t("Meridian Market · Demo workspace")}</p>
              <h2 className="mt-4 text-balance text-3xl font-medium tracking-[-0.04em] sm:text-5xl">{t("Follow every signal. Challenge every conclusion.")}</h2>
              <p className="mt-5 max-w-xl text-sm leading-7 text-muted">{t("Explore the full product story with deterministic demo data. No account, integration, or production system required.")}</p>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <Link href="/command" className="inline-flex h-12 items-center justify-center gap-2 rounded-lg bg-primary px-5 text-sm font-semibold text-primary-foreground">
                  {t("Enter Threadline")} <ArrowRight aria-hidden="true" className="size-4" />
                </Link>
                <Link href="/lab" className="inline-flex h-12 items-center justify-center gap-2 rounded-lg border border-danger/25 px-5 text-sm font-medium text-danger hover:bg-danger/[0.04]">
                  <Gamepad2 aria-hidden="true" className="size-4" /> {t("Run Crisis Lab")}
                </Link>
                <a href="https://github.com/junlee122-bot/something" className="inline-flex h-12 items-center justify-center gap-2 rounded-lg border border-border-strong px-5 text-sm font-medium hover:bg-white/[0.035]">
                  <Github aria-hidden="true" className="size-4" /> {t("View source")}
                </a>
              </div>
            </div>
          </div>
      ...
      <footer className="border-t border-border px-5 py-8 sm:px-8">
        <div className="mx-auto flex max-w-[1120px] flex-col gap-4 text-xs text-muted sm:flex-row sm:items-center">
          <Wordmark />
          <p className="sm:ms-auto">{t("Concept product · Deterministic demo data · Built for the portfolio")}</p>
          <a href="https://github.com/junlee122-bot/something" className="text-foreground hover:text-primary">{t("Source")}</a>
        </div>
      </footer>
    </div>
  );
